...
@st.cache_data(show_spinner=False)
def load_source_csv_preview(filename: str) -> pd.DataFrame:
    """Return the source CSV as a dataframe without app-specific derived columns."""
    return pd.read_csv(Path(__file__).resolve().parent / "data" / filename, comment="#")
...
with story_tab:
    ...
    story_country_df = (
        reported_case_totals_monthly_df.assign(
            Year=reported_case_totals_monthly_df["Date"].dt.year
        )
...
with about_tab:
    ...
    severity_fig.add_trace(
        go.Scatter(
            x=severity_split_monthly_df["Date"],
            y=severity_split_monthly_df["AI incident"],
            stackgroup="one",
            mode="lines",
            name="AI incident",
    ...
    severity_fig.add_trace(
        go.Scatter(
            x=severity_split_monthly_df["Date"],
            y=severity_split_monthly_df["AI hazard"],
            stackgroup="one",
            mode="lines",
            name="AI hazard",
